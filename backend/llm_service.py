...
class LLMService:
    ...
    def extract_thought_info(self, text: str) -> dict:
        ...
        user_msg = (
            ...
            f'  "unclear": 布尔值，判断输入是否清晰（见下方规则）\n'
            f'}}\n\n'
            f"unclear 字段判断规则：\n"
            f"- true（输入不清晰，以下任一情况）：\n"
            f"  * 残句，动作或目标缺失，例如「我有点想」「可不可以在家」「那个那个」\n"
            f"  * 纯噪音或识别明显错乱，例如「啊啊啊」「嗯嗯嗯」「几个不成词的字符」\n"
            f"  * 完全无法理解为一个想做的事/想去的地方/想说的话\n"
            f"- false（大多数情况，只要能猜出大致意图就应该是 false）：\n"
            f"  * 表达不完美但意图可理解，例如「画水彩」「去那个店」「给朋友发消息」→ false\n"
            f"  * 识别可能有误但意思猜得出，例如「换水彩」（猜是想画水彩）→ false\n"
            f"  * 信息不全但意图清晰，例如「我想去那个那个店」→ false\n\n"
            f"scene 字段说明（必须准确判断，以下例子是硬边界）：\n"
            f"- indoor（在家就能做的事 → 选这个）：\n"
            f"  烹饪、做菜、学做菜、烘焙；整理、打扫、收纳；画画、写字、阅读；\n"
            f"  打电话、视频通话；看片、听音乐；任何明确在家完成的事\n"
            f"- outdoor（需要离开家才能做的事 → 选这个）：\n"
            f"  去店铺、餐厅、咖啡馆；爬山、散步、跑步；看展、逛市集、逛街；\n"
            f"  去影院看电影；任何需要离开家才能完成的事\n"
            f"- either（哪里都能做，或真的无法判断 → 才选这个）：\n"
            f"  发消息、回邮件（家里和外面都行）；\n"
            f"  看书（在家或咖啡馆都行）；听播客（走路或在家都行）\n\n"
            f"念头：{text}"
        )
...
【硬规则：邀请只能围绕一个念头】
- 邀请话术必须只围绕被选中的那一个念头展开，严禁引入候选列表里其他念头的活动或场景。
- 严禁创造现实中不会同时发生的场景组合（例如"在公园做菜"、"在咖啡馆爬山"、"散步顺便学做菜"）。
- 如果被选中念头是 indoor（在家就能做的），话术不能暗示用户出门。
- 如果被选中念头是 outdoor（需要出门的），话术不能暗示用户在家完成。
